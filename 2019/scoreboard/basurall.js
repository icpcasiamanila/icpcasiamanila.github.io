...
function initScoreboard(options) {
    ...
    demoFetchData = (() => {
        ...
        const sc = 0.1;//0.6; // gaano kagaling
        const xeasy = 0.1;
        const newGuyProb = 1.0;//0.3;
        const xattprobs = {
            "A": 0.11*sc,
            "B": 0.03*sc,
            "C": 0.01*sc,
        ...
        async function demoFetchData(source) {
            ...
            console.log(xdata.contestants.length);
            if (0) {//xdata.contestants.length == 0) {
                // {
                //     "A": { score: 0, attempts: 0, penalty: 0, pending: false },
                // }
                for (const cont of defaultContestants) {
                    // const subs = {};
                    // for (const subid in cont.subs) {
                    //     subs[subid] = {
                    //         score: 0,//cont.subs[subid].score,
                    //         attempts: 0,//cont.subs[subid].attempts,
                    //         penalty: 0,//cont.subs[subid].penalty,
                    //         pending: 0,//cont.subs[subid].pending,
                    //     };
                    // }
                    // xdata.contestants.push({
                    //     name: cont.name,
                    //     rank: cont.rank,
                    //     score: cont.score,
                    //     attempts: cont.attempts,
                    //     penalty: cont.penalty,
                    //     subs: cont.subs,
                    // });
                    // new guy
                    const c = {
                    ...
                    for (const p of xdata.problems) {
                        c.subs[p] = {
                            score: cont.subs[p].score,
                            attempts: 0,
                            penalty: cont.subs[p].penalty,
                            pending: cont.subs[p].pending,
                        };
                    }
            ...
            while (Math.random() < 0.9 && xdata.contestants.length < 1111) {
                // new guy
                const c = {
                ...
                if (xdata.contestants.length >= 2) while (!trySolving(c));
            }
        ...
        methods: {
            ...
            processData(allData) {
                if (allData) {
                    ...
                    this.summary.attempts = allData.summary.attempts;
                    this.summary.score = allData.summary.score;
                    this.summary.penalty = allData.summary.penalty;
                    this.summary.subs = allData.summary.subs;
            ...
            classForSub(c, prob) {
                ...
                if (sub.pending) {
                    ...
                    const summarySub = this.summarySubs(prob);
                    return (summarySub && sub.penalty == summarySub.penalty ?
            ...
            subId(sub) {
                console.log("SUBID", `${sub.score}_${sub.attempts}_${sub.penalty}_${sub.pending}`);
                return "0";//`${sub.score}_${sub.attempts}_${sub.penalty}_${sub.pending}`;
            },
            getSummary() {
                let x = this.summary;
                if (0) {//} && this.summary.subs) {
                    return {
                        attempts: 0,
                        score: 0,
                        penalty: Number.POSITIVE_INFINITY,
                ...
                <transition-group name="scoreboard" tag="tbody">
                    <tr v-for="c in contestants" :key="c.name">
                        ...
                        <transition name="entry-value" mode="out-in" v-for="prob in problems" :key="prob">
                            <td :key="subId(c.subs[prob])" :class="['t-problem', classForSub(c, prob)]">
                                B
                            </td>
                        </transition>
                        <transition name="entry-value" mode="out-in">
